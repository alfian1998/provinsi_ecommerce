...
class Gridview extends CI_Controller{
	...
	function ajax($id=null,$p=null,$o=null,$category_id=null) {
		if($id == 'ses_kecamatan') {
		    ...
		    $html .=" 	function ses_kelurahan(i,k) {
					        $.get('".site_url('gridview/ajax/ses_kelurahan/'.$p.'/'.$o.'/'.$category_id)."?ses_kecamatan='+i+'&ses_kelurahan='+k,null,function(data) {
	...
	function ajax_all($id=null,$p=null,$o=null,$category_parent=null) {
		if($id == 'ses_kecamatan') {
		    ...
		    $html .=" 	function ses_kelurahan(i,k) {
					        $.get('".site_url('gridview/ajax_all/ses_kelurahan/'.$p.'/'.$o.'/'.$category_parent)."?ses_kecamatan='+i+'&ses_kelurahan='+k,null,function(data) {
					            $('#box_kelurahan').html(data.html);
					        },'json');
					    }";
			$html .='	});
	...
	function index($p=1, $o=0, $category_id=null) {	
		...
		//
		$data['paging'] = $this->product_model->paging_product_by_category_gridview($p,$o,$category_id);
		$data['list_product'] = $this->product_model->list_product_by_category_gridview($o, $data['paging']->offset, $data['paging']->per_page,$category_id);
		...
		$this->load->view('public/gridview/index', $data);
	...
	function all($p=1, $o=0, $category_parent=null) {	
		...
		$data['ses_kelurahan'] = @$_SESSION['ses_kelurahan'];
		//
		$data['paging'] = $this->product_model->paging_product_by_parent_category_gridview($p,$o,$category_parent);
		$data['list_product'] = $this->product_model->list_product_by_parent_category_gridview($o, $data['paging']->offset, $data['paging']->per_page,$category_parent);
		...
		$this->load->view('public/main/header', $data);		
		$this->load->view('public/main/top-menu');		
		$this->load->view('public/gridview/all', $data);
		$this->load->view('public/main/footer');
	}	
	...
	function search($p=null, $o=null, $category_id=null) {
		...
		//
		redirect('gridview/index/'.$p.'/'.$o.'/'.$category_id);
	}
	...
	function search_all($p=null, $o=null, $category_parent=null) {
		...
		$_SESSION['ses_kelurahan'] = ($ses_kelurahan != '') ? $ses_kelurahan : false;
		//
		redirect('gridview/all/'.$p.'/'.$o.'/'.$category_parent);
	}
