...
class Send extends CI_Controller{
	...
	function index($p=1, $o=0) {
		...
		$this->load->view('customer/status_deposit/send', $data);
	...
	function buyer($p=1, $o=0) {
		...
		//
		$paging = $this->checkout_model->paging_send_pay_customer($p,$o,$customer_id);
		$list_buyer = $this->checkout_model->get_send_pay_customer($o, $paging->offset, $paging->per_page,$customer_id);
		//
		$html = '';
		...
		$html.= '		<td align="center"><label class="label label-success" style="font-size: 13px;">'.$data['billing_id'].'</label></td>
                        ...
                        <td rowspan="3">
                        	<label class="label label-success" style="font-size: 13px;"><i class="fa fa-check"></i> Sudah Ditransfer</label><br><br>
                        	<label class="label label-primary" style="font-size: 13px;"><i class="fa fa-calendar"></i> '.convert_date_indo($data['bayar_customer_date']).'</label>
                        	<a href="'.site_url('assets/images/transfer_admin/'.$data['bayar_customer_img']).'" target="_blank" class="btn btn-primary btn-sm bold" style="margin-top: 10px;"><i class="fa fa-eye"></i> Cek Bukti Transfer</a>
                        </td>
                    </tr>';
        $html.= '	<tr>
						<th></th>
		...
		$html.='	        <li><a href="'.site_url("send/buyer/$paging->c_start_link/$o").'" class="link_pagination"><span><i class="fa fa-angle-double-left"></i></span></a></li>';
	                }
	                if($paging->prev){
	    $html.='            <li><a href="'.site_url("send/buyer/$paging->prev/$o").'" class="link_pagination"><span><i class="fa fa-angle-left"></i></span></a></li>';
	                }
	    ...
	    $html.='				<a href="'.site_url("send/buyer/$i/$o").'" class="link_pagination">'.$i.'</a>
	    					</li>';
	                        }

	                if($paging->next){
	    $html.='            <li><a href="'.site_url("send/buyer/$paging->next/$o").'" class="link_pagination"><span><i class="fa fa-angle-right"></i></span></a></li>';
	                        }
	                        
	                if($paging->end_link){
	    $html.='            <li><a href="'.site_url("send/buyer/$paging->c_end_link/$o").'" class="link_pagination"><span><i class="fa fa-angle-double-right"></i></span></a></li>';
	                        }
	    $html.='		</ul>
	                </td>
                </tr>
