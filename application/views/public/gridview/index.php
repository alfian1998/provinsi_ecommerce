<script type="text/javascript">
    $(function() {
        $('#ses_urutan,#ses_kabupaten,#ses_kecamatan,#ses_kelurahan').bind('change',function() {
            $('#form-search').attr('action','<?=site_url("gridview/search/".$p."/".$o."/".$category_id)?>').submit();
        });
        //
        ...
        function ses_kecamatan(i,k) {
            $.get('<?=site_url("gridview/ajax/ses_kecamatan/".$p."/".$o."/".$category_id)?>?ses_kabupaten='+i+'&ses_kecamatan='+k,null,function(data) {
			...
			<div class="col-xs-12 col-sm-4 col-md-3">
                ...
                <div class="block block-vertical-menu">
                    ...
                    <div class="vertical-menu-content">
                        <form class="form-inline" name="form-search" id="form-search" method="post" action="<?=site_url('gridview/search/'.$p.'/'.$o.'/'.$category_id)?>">
                        <table class="no-border">
                            <tr>
                                <td class="no-border">Tampilan</td>
                                <td class="no-border">:</td>
                                <td class="no-border">
                                    <a href="<?=site_url('listview/location/1/0/'.$category_id)?>" title="Tampilan List" class="button-small button-blue"><i class="fa fa-list"></i></a>
                            ...
                            <tr>
                                <td class="no-border" colspan="3">
                                    ...
                                    <a href="<?=site_url('gridview/location/'.$p.'/'.$o.'/'.$category_id)?>" class="btn btn-danger" style="width: 49%">Batal <i class="fa fa-refresh"></i></a>
                                </td>
                            </tr>
                        </table>
                        </form>
			...
			<div class="col-xs-12 col-sm-8 col-md-9">
				<div class="box-panel" style="margin-bottom: -20px;">
					<div class="panel panel-primary">
						<div class="panel-heading text-box-panel"><i class="fa fa-th"></i> (<?=$get_category_parent['category_nm']?>) <?=$get_category['category_nm']?></div>
					</div>
				</div>
                                    ...
                                    <div class="product-name-price">
                                        ...
                                        <div class="button-new-product">
                                            ...
                                            <?php else: ?>
                                                ...
                                                <a href="<?=site_url('web/details/'.$data['product_id'].'/'.$data['product_url'])?>" class="btn btn-sm btn-success span4"><i class="fa fa-shopping-cart"></i> Beli</a>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div style="text-align: right;">
                        <ul class="pagination" style="margin-top: 17px; margin-bottom: 0px;">
                            <?php if($paging->start_link): ?>
                                <li><a href="<?=site_url("gridview/index/$paging->c_start_link/$o/$category_id") ?>"><span><i class="fa fa-angle-double-left"></i></span></a></li>
                            <?php endif; ?>
                            <?php if($paging->prev): ?>
                                <li><a href="<?=site_url("gridview/index/$paging->prev/$o/$category_id") ?>"><span><i class="fa fa-angle-left"></i></span></a></li>
                            <?php endif; ?>

                            <?php for($i = $paging->c_start_link; $i <= $paging->c_end_link; $i++): ?>
                                <li <?php jecho($p, $i, "class='active'") ?>><a href="<?=site_url("gridview/index/$i/$o/$category_id") ?>"><?=$i ?></a></li>
                            <?php endfor; ?>

                            <?php if($paging->next): ?>
                                <li><a href="<?=site_url("gridview/index/$paging->next/$o/$category_id") ?>"><span><i class="fa fa-angle-right"></i></span></a></li>
                            <?php endif; ?>
                            <?php if($paging->end_link): ?>
                                <li><a href="<?=site_url("gridview/index/$paging->c_end_link/$o/$category_id") ?>"><span><i class="fa fa-angle-double-right"></i></span></a></li>
                            <?php endif; ?>
                        </ul>
                    </div>
                <?php endif;?>
